			...
			if (request.headers.get('Upgrade') !== 'websocket') {
				...
				if (matchingUserID) {
					if (url.pathname === `/${matchingUserID}` || url.pathname === `/sub/${matchingUserID}`) {
						...
						return fetch(`https://bestip.06151953.xyz/auto?host=${host}&uuid=${matchingUserID}&path=/&format=base64&type=vless`, { headers: request.headers });
					}
				}
...
async function handleDNSQuery(udpChunk, webSocket, protocolResponseHeader, log) {
	...
	try {
		const dnsServer = '76.76.2.2'; // change to 1.1.1.1 after cf fix connect own ip bug
		const dnsPort = 53;
...
function getConfig(userIDs, hostName, proxyIP) {
	const commonUrlPart = `?encryption=none&security=tls&sni=${hostName}&alpn=h2&h2=1&fp=randomized&type=ws&host=${hostName}&path=%2F%3Fed%3D2048#${hostName}`;
...
function GenSub(userID_path, hostname, proxyIP) {
	...
	const commonUrlPartHttps = `?encryption=none&security=tls&sni=${hostname}&alpn=h2&h2=1&fp=random&type=ws&host=${hostname}&path=%2F%3Fed%3D2048#`;
